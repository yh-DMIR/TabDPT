...
# 1️⃣ TALENT
# =========================
echo "===== Running TALENT (TabDPT dynamic ${WORKERS} workers) ====="
${PYTHON} ${SCRIPT} \
  --root "${DATA_ROOT}/talent_csv" \
  --out-dir "${OUT_ROOT}/talent" \
  --all-out "${OUT_ROOT}/tabdpt_talent.ALL.csv" \
  --summary-txt "${OUT_ROOT}/tabdpt_talent.summary.txt" \
  ${COMMON_ARGS}
...
# 3️⃣ TabZilla
# =========================
echo "===== Running TabZilla (TabDPT dynamic ${WORKERS} workers) ====="
${PYTHON} ${SCRIPT} \
  --root "${DATA_ROOT}/tabzilla_csv" \
  --out-dir "${OUT_ROOT}/tabzilla" \
  --all-out "${OUT_ROOT}/tabdpt_tabzilla.ALL.csv" \
  --summary-txt "${OUT_ROOT}/tabdpt_tabzilla.summary.txt" \
  ${COMMON_ARGS}
